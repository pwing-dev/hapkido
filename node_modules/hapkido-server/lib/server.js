...
const assets          = require('hapkido-server/lib/static');
const router          = require('hapkido-server/lib/routes/router');

const AppState        = require('hapkido-core/models/application');
const setuptool       = require('hapkido-server/lib/setuptool');
...
const createApp = () => new Promise((resolve, reject) => AppState.assertInitialized((error, Application) => {
  ...
  try {
    ...
    // IP Filter
    app.use(ipFilter(cfg('whitelist'), {
      mode: 'allow',
      log: cfg('debug.verbosity') > 0,
    }));

    // Reverse Proxy
    if (cfg('reverseProxy')) {
      app.set('trust proxy', cfg('reverseProxy'));
    }
    ...
    const frontendDir = path.normalize(path.join(__dirname, '../frontend'));
